...
function CorrectAnswer() {
  ...
  document.getElementById("img_here").src="https://raw.githubusercontent.com/cheesycat69/daily_dose/main/july_25/beautiful.jpeg";
  document.getElementById("video").src="https://www.youtube.com/embed/MrTz5xjmso4?autoplay=1";
  document.getElementById("video_container").style.display="block";
  document.getElementById("video_heading").innerHTML="<h1>Sean Kingston - Beautiful Girls</h1>";
}
...
function ValidateAnswer(){
  ...
  for(var i = 0; i < letterArr.length; i++){
    var inputbox = letterArr[i].getElementsByClassName('char')[0];
    var idx = Number(letterArr[i].getAttribute("data-x"));
    var idy = Number(letterArr[i].getAttribute("data-y"));
    if( inputbox.value.toUpperCase() != board[idx][idy]){
      flag=1;
      inputbox.classList.add("wrongbox")
      inputbox.classList.remove("rightbox")
      console.log(board[idx][idy], idx, idy, inputbox.value);
    } else {
      inputbox.classList.add("rightbox")
      inputbox.classList.remove("wrongbox")
      console.log(board[idx][idy], idx, idy, inputbox.value);
    }
  }
  if(flag==0){
...
function BoardCharToElement(c, j, i){
  ...
  return EleStr('div',[{a:'class',v:arr}],c, j, i);
}
...
function RegisterEvents(){
  ...
  document.getElementById("btnCheck").addEventListener('click',ValidateAnswer,false);
}
RegisterEvents();
...
function EleStr(e,c,h, idx, idy){
  h = (h)?h:"";
  for(var i=0,s="<"+e+" "; i<c.length; i++){
    s+=c[i].a+ "='"+ArrayToString(c[i].v," ")+"' ";    
  }
  return (s+"data-x="+idx+" data-y="+idy+">"+h+"</"+e+">");
}
